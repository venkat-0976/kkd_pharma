...
import { UnionShopIdentity } from "@/features/member/UnionShopIdentity";
...
import { memberNav, mobileNav, roleNav } from "@/config/memberNav";
import { cn } from "@/lib/utils";
import { authService } from "@/services/auth/auth.service";
import { useMemberRecord } from "@/hooks/useMemberRecord";
import { buildAlerts } from "@/services/member/member.service";
import { getUnionByMemberId } from "@/config/unions";
import { UnionDetailsPopover } from "@/layouts/AuthenticatedPortalLayout";
...
export function MemberShell({ children }: { children: ReactNode }) {
  ...
  const customNav =
    session?.loginType && session.loginType in roleNav
      ? roleNav[session.loginType as keyof typeof roleNav]
      : memberNav;
  const bottomNav = customNav === memberNav ? mobileNav : customNav.slice(0, 4);

  const alerts = record ? buildAlerts(record).filter((a) => a.status !== "valid") : [];
  const shopId = record?.profile.memberId ?? authService.getSession()?.memberId ?? "";
  const shopName = record?.profile.shopName ?? "Member workspace";
  ...
  const whatsAppUrl = union
    ? `https://wa.me/918842001180?text=${encodeURIComponent(`Hello ${union.name}, I need assistance.`)}`
    : undefined;
    ...
    <div className="min-h-screen bg-surface/40">
      <header className="sticky top-0 z-40 border-b border-border bg-secondary">
        <div className="mx-auto flex h-16 w-full max-w-[110rem]">
          ...
          >
            {sidebarCollapsed ? (
              <UnionShopIdentity shopId={shopId} compact markOnly />
            ) : (
              <Logo name="PharmaConnect" subtitle="State Pharmacy Network" />
            )}
            <button
              type="button"
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-56">
                  <DropdownMenuLabel className="space-y-0.5">
                    <p className="truncate">{record?.profile.shopName ?? "Member"}</p>
                    {shopId ? (
                  ...
                  <DropdownMenuItem asChild>
                    <Link to="/member/settings">Settings</Link>
                  </DropdownMenuItem>
                  <DropdownMenuSeparator />
...
export function ShellButton(props: React.ComponentProps<typeof Button>) {
  return <Button {...props} />;
}
