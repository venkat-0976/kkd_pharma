      ...
      <div className="flex flex-wrap items-center justify-end gap-2">
        ...
        >
          <button
            ...
            className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold transition-colors ${!tableView ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground"}`}
          >
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button type="button" variant="outline" size="sm">
                <Columns3 className="size-4" aria-hidden="true" /> Columns
              </Button>
            </PopoverTrigger>
              ...
              <tr className="border-b border-primary/15">
                {visibleColumns.map((column) => (
                  <th
                    key={column.key}
                    scope="col"
                    className="whitespace-nowrap px-4 py-3 text-xs font-bold uppercase tracking-[0.08em] text-foreground"
                  >
                    {column.label}
                  </th>
                ))}
