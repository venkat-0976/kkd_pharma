    ...
    <section className="flex h-full flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-soft">
      ...
      <header className="grid grid-cols-[minmax(0,1fr)_4.25rem] items-start gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 line-clamp-2 text-xs leading-relaxed text-secondary-foreground/80">{description}</p>
        </div>
        <span
          className={cn(
        ...
        <>
          <dl className="mt-3 divide-y divide-border/80 border-y border-border/80">
            {fields.map((field) => {
              ...
              return (
                <div key={String(field.key)} className="grid grid-cols-[7.25rem_minmax(0,1fr)] items-center gap-3 py-1.5">
                  ...
                  <dd className="truncate text-right text-sm font-semibold text-foreground">
                    {field.type === "date" ? (
                      ...
                      <span className="font-medium text-secondary-foreground/70">Not provided</span>
                    )}
                  </dd>
                </div>
              );
          ...
          <div className="mt-3 space-y-1.5">
            ...
            {older.length > 0 ? (
              <>
                ...
                >
                  {showOlder ? "Hide earlier files" : `${older.length} earlier file${older.length === 1 ? "" : "s"}`}
                </button>
                {showOlder
                  ? older.map((file) => (
                      <DocumentRow
          ...
          <div className="mt-auto flex flex-wrap items-center justify-between gap-2 pt-3">
            <p className="inline-flex items-center gap-2 text-xs font-semibold text-secondary-foreground">
              <span className={cn("size-1.5 rounded-full", footerDot[status])} aria-hidden="true" />
              {statusLabel[status]}
            </p>
