...
import { employeeSchema, type EmployeeValues } from "@/utils/validation/memberForms";
...
export function EmployeeDialog({ open, onOpenChange, employee, onSubmit }: EmployeeDialogProps) {
  const { register, handleSubmit, reset, setValue, watch, formState } = useForm<EmployeeValues>({
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90dvh] overflow-y-auto sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Staff records stay private and are used for your shop records only.
          </DialogDescription>
        </DialogHeader>
        <form className="space-y-4" onSubmit={handleSubmit(onSubmit)} noValidate>
          <PersonPhotoUpload
            name={watch("fullName")}
            value={watch("photo")}
            onChange={(photo) => setValue("photo", photo, { shouldDirty: true })}
          />
          <div>
            <Label htmlFor="emp-name">Employee name</Label>
            <Input id="emp-name" className="mt-1.5" {...register("fullName")} />
          ...
          <div>
            ...
            <Input id="emp-mobile" inputMode="numeric" className="mt-1.5" {...register("mobile")} />
          ...
          <div>
            ...
            <Textarea id="emp-address" rows={3} className="mt-1.5" {...register("address")} />
