...
export function BloodBankDetailsPage() {
  ...
  const fields = ["Blood bank name", "Blood bank code", "Operating authority", "License number", "Emergency contact"];
  return (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {fields.map((field) => (
            <div key={field}>
              ...
              <Input className="mt-2" defaultValue={field === "Blood bank name" ? session?.shopName : undefined} placeholder={`Enter ${field.toLowerCase()}`} />
            </div>
          ))}
        </div>
      </section>
