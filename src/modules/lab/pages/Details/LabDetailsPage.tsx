...
export function LabDetailsPage() {
  ...
  const fields = ["Laboratory name", "Laboratory code", "Laboratory type", "Registration number", "Accreditation"];
  return (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {fields.map((field) => (
            <div key={field}>
              ...
              <Input className="mt-2" defaultValue={field === "Laboratory name" ? session?.shopName : undefined} placeholder={`Enter ${field.toLowerCase()}`} />
            </div>
          ))}
        </div>
      </section>
      <p className="flex items-center gap-2 text-xs text-muted-foreground">
        <Pencil className="size-3.5" aria-hidden="true" /> Changes are saved to your private workspace and reviewed by authorised union administrators.
      </p>
    </div>
  );
}
