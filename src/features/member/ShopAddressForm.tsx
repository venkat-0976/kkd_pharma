  ...
  if (navigator.permissions?.query) {
    try {
      ...
      if (status.state === "denied") {
        throw new Error("Location access is blocked. Allow location for this site in your browser settings.");
      }
    } catch (error) {
...
export function ShopAddressForm({ address }: { address: ShopAddress }) {
  ...
  const fillFromPostalPlace = (place: ReverseGeocodedAddress | null, lookupKey: string, failure: string) => {
    ...
    toast.success("Address updated from PIN code", {
      description: "Area, mandal, district and state were filled from India Post. Door number, street and city were left empty for you to enter.",
    });
    return true;
  };

  const fillAddressFromCoordinates = useCallback(async (latitude: number, longitude: number) => {
  ...
  const fillAddressFromPincode = useCallback(async (pincode: string) => {
    ...
    try {
      ...
      fillFromPostalPlace(
        place,
        key,
        "No postal records were found for this PIN code.",
      );
    } catch (error) {
      toast.error("PIN lookup failed", { description: geoErrorMessage(error) });
    }
      ...
      >
        <div className="space-y-8">
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">Enter these by hand after auto-fill.</p>
          ...
          <div className="rounded-2xl border border-border/80 bg-surface/50 p-4 sm:p-5">
            <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-1 text-xs leading-relaxed text-muted-foreground sm:text-sm">
                  Use my location or a PIN code to fill area, mandal, district and state from India Post.
                  Door number, street and city stay empty for you to enter, then click Save address.
                </p>
              </div>
              <Button type="button" variant="outline" size="sm" onClick={() => void detectLocation()} disabled={locating}>
                {locating ? (
            ...
            <div className="mt-5 grid gap-4 sm:grid-cols-2">
              <div>
                <Label htmlFor="latitude" className="text-[11px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">
                  Latitude
                </Label>
                <Input id="latitude" className="mt-2 h-11 rounded-lg" inputMode="decimal" {...form.register("latitude")} />
              </div>
              <div>
                <Label htmlFor="longitude" className="text-[11px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">
                  Longitude
                </Label>
                <Input id="longitude" className="mt-2 h-11 rounded-lg" inputMode="decimal" {...form.register("longitude")} />
              </div>
            </div>
