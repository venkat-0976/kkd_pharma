...
} from "@/features/member/DocumentUploadDialog";
...
export function PharmacistDialog({
  ...
  const { register, handleSubmit, reset, getValues, formState } = useForm<PharmacistValues>({
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-border bg-surface/60 p-3">
                ...
                {pharmacist?.documentId ? (
                  <Button
                    ...
                    disabled={!documents.some((item) => item.id === pharmacist.documentId)}
                  >
