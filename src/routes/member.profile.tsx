...
import { SectionCard, FieldList } from "@/features/member/SectionCard";
...
export const Route = createFileRoute("/member/profile")({
  head: () => ({
    meta: [
      { title: "My Profile — Kakinada Union Member Portal" },
      { name: "description", content: "Private business profile for Kakinada Union members." },
  ...
  return (
    <div className="space-y-6">
      <section className="overflow-hidden rounded-2xl border border-border bg-card shadow-soft">
        ...
        <div className="flex min-h-44 flex-col gap-6 p-6 sm:flex-row sm:items-center sm:justify-between sm:p-8">
          <div className="flex min-w-0 items-center gap-5">
            <span className="grid size-20 shrink-0 place-items-center rounded-3xl bg-primary text-2xl font-bold tracking-wide text-primary-foreground shadow-soft sm:size-22 sm:text-3xl">
            ...
            <div className="min-w-0">
              ...
              <div className="mt-4 flex flex-wrap gap-2.5">
                <span className="rounded-full border border-border bg-surface/80 px-3 py-1.5 text-xs font-semibold text-foreground">
                  {profile.memberType}
                </span>
                <span className="rounded-full border border-border bg-surface/80 px-3 py-1.5 text-xs font-semibold text-foreground">
                  {profile.memberId}
          ...
          <Button className="shrink-0 self-start sm:self-center" variant={editing ? "outline" : "default"} onClick={() => setEditing((v) => !v)}>
