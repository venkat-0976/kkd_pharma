...
function EmployeesPage() {
  ...
  const columns: MemberTableColumn<Employee>[] = [
    { key: "name", label: "Name", render: (employee) => <span className="font-semibold">{employee.fullName}</span> },
    { key: "mobile", label: "Mobile number", render: (employee) => employee.mobile },
    { key: "address", label: "Address", render: (employee) => employee.address },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      {record.employees.length === 0 ? (
        ...
        <MemberViewToggle rows={record.employees} columns={columns} cards={<div className="grid gap-4 md:grid-cols-2">
          {record.employees.map((employee) => (
            <article key={employee.id} className="overflow-hidden rounded-2xl border border-border bg-card shadow-soft">
              <div className="flex items-center gap-3 px-4 py-3">
                <PersonAvatar name={employee.fullName} photo={employee.photo} className="size-12 rounded-2xl" />
                <h2 className="min-w-0 flex-1 truncate font-display text-sm font-bold">{employee.fullName}</h2>
