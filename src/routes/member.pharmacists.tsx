...
import { StatusBadge } from "@/features/member/StatusBadge";
...
export const Route = createFileRoute("/member/pharmacists")({
  head: () => ({
    meta: [
      { title: "Pharmacists — Kakinada Union Member Portal" },
      { name: "description", content: "Manage registered pharmacists attached to your pharmacy." },
...
function PharmacistsPage() {
  ...
  const columns: MemberTableColumn<Pharmacist>[] = [
    { key: "name", label: "Name", render: (pharmacist) => <span className="font-semibold">{pharmacist.fullName}</span> },
    { key: "mobile", label: "Mobile number", render: (pharmacist) => pharmacist.mobile },
    { key: "licence", label: "Licence number", render: (pharmacist) => pharmacist.licenceNumber },
    { key: "expiry", label: "Licence expiry", render: (pharmacist) => formatDate(pharmacist.licenceExpiry) },
    { key: "address", label: "Address", render: (pharmacist) => pharmacist.address },
  ];
  ...
  return (
    <div className="space-y-6">
      <MemberPageHeader
        title="Pharmacists"
        description="Registered pharmacists and their licence validity. This information stays private."
        action={
          <Button
            onClick={() => {
              setEditing(null);
      ...
      {record.pharmacists.length === 0 ? (
        <EmptyState
          title="No pharmacists recorded"
          description="Add your registered pharmacist so licence expiry reminders can be sent to you."
        />
      ) : (
        <MemberViewToggle rows={record.pharmacists} columns={columns} cards={<div className="grid gap-4 md:grid-cols-2">
      ...
      <PharmacistDialog
        ...
        onUploadDocument={async (values, input) => {
          ...
          const { pharmacistValues, ...documentInput } = input;
          await memberService.uploadDocument({
            ...documentInput,
            ...(input.category === "Pharmacist Licence" && pharmacistValues
              ? {
                  pharmacistValues: {
                    ...pharmacistValues,
                    photo: pharmacistValues.photo ?? "",
          ...
          await invalidate();
          setDialogOpen(false);
          toast.success("Pharmacist document uploaded");
        }}
        onSubmit={async (values) => {
          ...
          await invalidate();
          setDialogOpen(false);
          toast.success(values.id ? "Pharmacist updated" : "Pharmacist added");
        }}
      />
      ...
      <AlertDialog open={!!pendingDelete} onOpenChange={(o) => !o && setPendingDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Their licence reminders will stop. You can add the pharmacist again at any time.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
