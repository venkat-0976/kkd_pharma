...
export async function resolvePinFromCoordinates(
  latitude: number,
  longitude: number,
): Promise<PinHint> {
...
function pickOffice(offices: PostalOffice[], hints: string[]) {
  const ranked = [...offices].sort(
    (left, right) => officeScore(right, hints) - officeScore(left, hints),
  );
  return ranked[0];
}
...
async function fetchPostOffices(pincode: string): Promise<PostalOffice[]> {
  const payload = await fetchJson<PostalPinResponse[]>(
